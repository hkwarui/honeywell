        ...
        <div class="row-fluid">
            ...
            <div class="span10">
                ...
                <div style="margin-top: -19px; margin-bottom: 21px;">
                    <?php
$result = $db->prepare("SELECT * FROM customer ORDER BY customer_name DESC");
$result->execute();
$rowcount = $result->rowcount();
?>
                    <div style="text-align:center;">
                        Total Number of Customers: <font color="green" style="font:bold 22px 'Aleo';">
                            <?php echo $rowcount; ?>
                        </font>
                    </div>
                </div>
          ...
          $result = $db->prepare("SELECT customer.*,  SUM(credits.amount) as total  FROM customer LEFT JOIN credits ON credits.id_number = customer.id_number GROUP BY customer.id_number ORDER BY total DESC");
          $result->execute();
        for ($i = 0; $row = $result->fetch(); $i++) {
         ?>
                        <tr class="record">
                            ...
                            <td><a href="editcustomer.php?id=<?php echo $row['id_number']; ?>"
                                    title="Click To Edit Customer" rel="facebox" style="color:blue"><i
                                        class="icon-edit"> </i>
                                </a> <a href="#" id="<?php echo $row['id_number']; ?>" class="delbutton"
                                    title="Click To Delete" style="color:red"><i class="icon-trash"></i></a>
                            </td>
                        </tr>
                        <?php
}
